...
real_T q0[4] = {0.5, 0.5, -0.18301270189221924,
...
static void rate_scheduler(RT_MODELplant *const rtM);
...
static void rate_scheduler(RT_MODELplant *const rtM)
{
  ...
  if ((rtM->Timing.TaskCounters.TID[1]) > 199)
...
void Plant::qtoQ(const real_T rtu_q[4], real_T rty_Q[9])
{
  ...
  {
    rty_Q[i] = (2.0 * rtu_q_0[i] - tmp[i] * rtu_q_1 * 2.0) + rtConstPplant.pooled6[i] * rtb_Sum1;
  }
...
void Plant::step()
{
  ...
  rtb_TmpSignalConversionAtMatr_0 = 1.0 / (rtb_qd4 * rtb_qd4) * (m * -3.983324E+14);
  ...
  rtb_DiscreteTimeIntegrator1 = 2.0 * rtb_TmpSignalConversionAtMatr_0 * std::cos(localProduct);

  // Product: '<S4>/Divide4' incorporates:
  //   Trigonometry: '<S4>/Trigonometric Function4'

  rtb_DiscreteTimeIntegrator2 = rtb_TmpSignalConversionAtMatr_0 * std::sin(localProduct);
  ...
  for (i = 0; i < 3; i++)
  {
    rtb_Sum[i] = (rtb_Sum8[i + 3] * rtb_TmpSignalConversionAtMatr_0 + rtb_Sum8[i] * rtb_qd4) + rtb_Sum8[i + 6] * rtb_DiscreteTimeIntegrator1;
  }
  ...
  rtb_Product_p_tmp = rtb_Sum[2] - (rtDW.DiscreteTimeIntegrator_DSTATE *
                                        rtb_Product_j[1] -
                                    rtDW.DiscreteTimeIntegrator1_DSTATE * rtb_Product_j[0]);
  ...
  for (i = 0; i < 3; i++)
  {
    rtb_Product_j[i] = (rtb_Sum8[i + 3] * rtb_DiscreteTimeIntegrator3 +
                        rtb_Sum8[i] * rtb_DiscreteTimeIntegrator2) +
                       rtb_Sum8[i + 6] * rtb_Product_p_tmp;
  }
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    rtY.magneticfield[i] = (rtb_Sum8[i + 3] * rtb_TmpSignalConversionAtMatr_0 +
                            rtb_Sum8[i] * rtb_qd4) +
                           rtb_Sum8[i + 6] * rtb_DiscreteTimeIntegrator1;
